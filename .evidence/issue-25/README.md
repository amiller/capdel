...
booted by `run-vm.sh` on branch `ready-25` @ `0d9b95a`; `capture.py` ran ON the VM
through an exec capability, and every probe is black-box HTTP against the live broker
at 127.0.0.1:4571 — the same rig and style as `.evidence/issue-8/` (#23).

| acceptance bullet | where demonstrated |
|---|---|
| an exec cap with a disk I/O quota | mint with `disk_max_bps: 4194304` — transcript §1 |
| throttled by cgroup-v2 io.max when exceeded | 16 MiB O_SYNC write: **0.02 s** unthrottled → **3.99 s** at 4 MiB/s; attenuated child (1 MiB/s) writes 4 MiB in **3.98 s** — 16/4 == 4/1 elapsed is the rate-limiting signature, not fixed latency |
| shown in a test, same rig as test/kernel.py | `test/kernel.py` io.max case (dd `oflag=direct`, `/var/tmp` on the root disk) → PASS on the VM — transcript §3 |
